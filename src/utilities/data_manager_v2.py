...
class DataManager(object):
    ...
    def get_intensity_volume_filepath(stack, downscale=32):
        ...
        vol_fn = os.path.join( DataManager.get_volume_root_folder(stack), basename, basename + '.bp')
        return vol_fn
    ...
    def get_annotation_as_score_volume_filepath(stack, downscale, structure):
        ...
        vol_fn = os.path.join( DataManager.get_volume_root_folder(stack), basename, 'score_volumes', basename + '.bp')
        return vol_fn
...
#         return grad_fp
    ...
    def get_original_volume_filepath_v2(stack_spec, structure=None, resolution=None):
        ...
        if stack_spec['vol_type'] == 'score':
            return os.path.join( DataManager.get_volume_root_folder(stack_spec['name']), vol_basename, 'score_volumes', vol_basename_with_structure_suffix + '.bp')
        elif stack_spec['vol_type'] == 'annotationAsScore':
            return os.path.join( DataManager.get_volume_root_folder(stack_spec['name']), vol_basename, 'score_volumes', vol_basename_with_structure_suffix + '.bp')
        elif stack_spec['vol_type'] == 'intensity':
            return os.path.join( DataManager.get_volume_root_folder(stack_spec['name']), vol_basename, vol_basename + '.bp')
        else:
            raise Exception("vol_type of %s is not recognized." % stack_spec['vol_type'])
