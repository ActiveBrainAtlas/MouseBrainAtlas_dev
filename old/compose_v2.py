...
Usage 1: compose.py --input_spec in.ini --prep_id alignedPadded
...
ini_fp = os.path.join(DATA_ROOTDIR, 'domain_specs', args.prep_id + '.ini')
operation_spec = load_ini(ini_fp)
assert operation_spec['type'] == 'warp', "Prep %s must be warp." % args.prep_id

elastix_output_dir = operation_spec['elastix_output_dir']
custom_output_dir = operation_spec['custom_output_dir']
toanchor_transforms_fp = operation_spec['transforms_csv']
anchor_img_name = operation_spec['anchor_image_name']
base_prep_id = operation_spec['base_prep_id']
