...
class init_GUI(QWidget):
    ...
    def initUI(self):
        ...
        # Button Text Field
        self.b1 = QPushButton("Select CZI file")
        ...
        # Button Text Field
        self.b2 = QPushButton("Select TIFF output folder")
...
You can choose to convert just the selected file, or all files in its folder using the top-right checkbox." )
    ...
    def checkboxPress(self, checkbox):
        ...
        if checkbox == self.cb1:
            ...
            for cb_to_disable in [self.cb_c_0, self.cb_c_1, self.cb_c_2]:
                cb_to_disable.setChecked(False)
        
        elif checkbox in [self.cb_c_0, self.cb_c_1, self.cb_c_2] :
    ...
    def buttonPress(self, button):
        ...
        if button == self.b1:
            fp = get_selected_file( initialdir='/media/alexn/Data_2/czi_convert_test/',
                                   default_filetype=[("czi files","*.czi"),("all files","*.*")] )
        ...
        # Button: "select TIFF output folder"
        if button == self.b2:
            fp = get_selected_folder( initialdir='/media/alexn/Data_2/czi_convert_test/' )
            self.tiff_destination = fp
    ...
    def validate_selections(self):
        ...
        if self.cb_c_all.isChecked():
            for i, cb in enumerate([self.cb_c_0, self.cb_c_1, self.cb_c_2]):
